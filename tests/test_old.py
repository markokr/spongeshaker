...
def test_copy():
    ...
    >>> x = sha3_256(b"The quick brown fox jumps over the lazy dog")
    >>> y = x.copy()
    >>> z = x.copy()
...
def test_prng():
    ...
    >>> x = KeccakPRNG(1024)
    >>> x.add_entropy(b"The quick brown fox jumps over the lazy dog.")
...
def test_keccac_encryption():
    r"""
    Keccak Encryption

    >>> ctx = SpongeWrap(1536)
    >>> ctx.add_header(b"password")
    >>> tohex(ctx.encrypt_body(b"Secret message"))
